...
// Created : Sunday, 27 April 2008 7:36:09 PM
//////////////////////////////////////////////////////////////////////////////
...
namespace shader
{
	...
	namespace detail
	{
#include "hlsl/system_ui.h"
	}

	system_ui::system_ui(dx9::render_interface *ri) :
		m_ri(ri)
	{
		...
		m_vertex_shader = compiler.compile_vertex_shader(detail::system_ui, strlen(detail::system_ui), "vs_main");		
		m_pixel_shader = compiler.compile_pixel_shader(detail::system_ui, strlen(detail::system_ui), "ps_main");
	}
	...
	int system_ui::get_num_input_semantics() const
	{
		return 3;
	}
	
	const vertex_format::semantic_decl* system_ui::get_input_semantic(int i) const
	{
		if(i > 2)
			return 0;
	...
	const graphics::shader::parameter_buffer_ptr system_ui::get_shader_parameters() const
	{
		return gs::parameter_buffer_ptr();	
	}
	
	graphics::shader::parameter_buffer_ptr system_ui::create_default_table() const
	{
	...
	const graphics::shader::parameter_buffer_layout_ptr system_ui::get_parameter_layout() const
	{
		return m_layout;	
	}
	
	void system_ui::render_setup(graphics::shader::parameter_buffer_ptr pb)
	{
		// setup vertex shader
		gs::parameter_buffer_view view(pb, m_layout);
